...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
        ...
        # self.important_sampling = True
        # self.reservoir_retention = False
        # self.td_retention = False
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            #if self.prioritized:
            experiences, ISWeights, sample_index = self.memory.sample(self.batch_size)
            # else:
            #     experiences, sample_index = self.memory.sample(self.batch_size)
            ...
            # #######
            # q_next = q_batch
            # q_eval = self.model.predict(state0_batch, batch_size=self.batch_size)

            # # change q_target w.r.t q_eval's action
            # q_target = q_eval.copy()

            # batch_index = np.arange(self.batch_size, dtype=np.int32)
            # eval_act_index = (action_batch)
            # reward = reward_batch

            # for i in range(self.batch_size):
            #     q_target[i, eval_act_index[i]] = reward[i] + self.gamma * q_next[i]

            
            # self.tpTd = abs(np.sum(q_eval - q_target,axis=1))
            
            #print(batch_memoryOLast)
            ...
            #######
            #tploss = self.trainable_model.predict(ins + [targets, masks])[0]

            #print("loss:",tploss)
            #print(ISWeights)
            metrics = self.trainable_model.train_on_batch(ins + [targets, masks], [dummy_targets, targets], sample_weight={"loss": ISWeights})
            #print("len", self.trainable_model.metrics_names)
            self.tpTd = self.trainable_model.predict(ins + [targets, masks])[0]
            self.memory.batch_update(sample_index, self.tpTd)
